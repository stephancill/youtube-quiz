...
export class YoutubeService {
  ...
  async listRecentWatchedVideos(
    ...
    const runtimeContext = this.extractRuntimeRequestContext(firstHistoryResponse.html);
    console.log(
    ...
    const isAuthOrConsentResponse = videos.length === 0 && this.looksLikeAuthOrConsentPage(html);
  ...
  private persistCookieJarIfSafe(input: {
    ...
    if (JSON.stringify(input.originalCookieJar) === JSON.stringify(input.nextCookieJar)) {
      return;
    }
    ...
    const containsCriticalDeletion = deletedNames.some((name) => CRITICAL_AUTH_COOKIES.has(name));
  ...
  private applyMergedCookieJar(cookieJar: YoutubeCookieJar, headers: Headers): YoutubeCookieJar {
  ...
  private async warmupGuideRequests(input: {
    ...
    for (let requestIndex = 0; requestIndex < GUIDE_WARMUP_REQUESTS; requestIndex += 1) {
  ...
  private extractRuntimeRequestContext(html: string): RuntimeRequestContext {
    ...
    const clientVersion =
      this.extractStringFromHtml(html, /"INNERTUBE_CONTEXT_CLIENT_VERSION"\s*:\s*"([^"]+)"/) ??
      this.extractStringFromHtml(html, /"INNERTUBE_CLIENT_VERSION"\s*:\s*"([^"]+)"/) ??
      DEFAULT_CLIENT_VERSION;
    const clientName =
      this.extractStringFromHtml(html, /"INNERTUBE_CONTEXT_CLIENT_NAME"\s*:\s*"([^"]+)"/) ??
      DEFAULT_CLIENT_NAME;
    const sessionIndex = this.extractStringFromHtml(html, /"SESSION_INDEX"\s*:\s*"([^"]+)"/) ?? "0";
    const visitorData = this.extractStringFromHtml(html, /"VISITOR_DATA"\s*:\s*"([^"]+)"/) ?? null;
  ...
  private logAuthOrConsentDiagnostics(input: {
    ...
    const hasAccountsGoogleSignin = /accounts\.google\.com\/signin/i.test(input.html);
  ...
  private mergeCookieJarFromResponseHeaders(
    ...
    for (const setCookieHeader of setCookieHeaders) {
      ...
      if (JSON.stringify(previousCookie) !== JSON.stringify(parsedCookie.cookie)) {
        stats.updated += 1;
        stats.updatedNames.push(parsedCookie.name);
      }
    }

    if (JSON.stringify(nextCookieJar) === JSON.stringify(currentCookieJar)) {
      return null;
    }
  ...
  private parseSetCookie(
    ...
    for (const attribute of attributes) {
      ...
      const attributeName = (
        attributeSeparatorIndex === -1 ? attribute : attribute.slice(0, attributeSeparatorIndex)
      )
        .trim()
        .toLowerCase();
      const attributeValue =
        attributeSeparatorIndex === -1 ? "" : attribute.slice(attributeSeparatorIndex + 1).trim();
  ...
  private extractVideosFromHistoryData(data: unknown): YoutubeVideo[] {
    ...
    while (stack.length > 0) {
      ...
      if (maybeRenderer.videoRenderer && typeof maybeRenderer.videoRenderer === "object") {
        ...
        const title =
          typeof video.title?.runs?.[0]?.text === "string" ? video.title.runs[0].text : null;
        const channelTitle =
          typeof video.ownerText?.runs?.[0]?.text === "string"
            ? video.ownerText.runs[0].text
            : "Unknown channel";
        const lengthText =
          typeof video.lengthText?.simpleText === "string" ? video.lengthText.simpleText : null;
      ...
      if (maybeLockup.lockupViewModel && typeof maybeLockup.lockupViewModel === "object") {
        ...
        const title =
          typeof lockup.metadata?.lockupMetadataViewModel?.title?.content === "string"
        ...
        const watchedRatio = this.percentToRatio(
          lockup.contentImage?.thumbnailViewModel?.overlays?.[0]?.thumbnailBottomOverlayViewModel
            ?.progressBar?.thumbnailOverlayProgressBarViewModel?.startPercent,
        );
  ...
  private looksLikeAuthOrConsentPage(html: string): boolean {
    ...
    const hasAuthFlowMarker =
      /ServiceLogin|accounts\.google\.com\/signin|consent\.youtube\.com/i.test(html);
