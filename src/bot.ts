...
export class QuizBot {
  ...
  private registerHandlers() {
    ...
    this.bot.callbackQuery(/^hint:(\d+):(\d+):(\d+)$/, async (ctx) => {
      ...
      const contextualHint =
        question.hint ?? "Focus on the key claim or example discussed in the question.";
  ...
  private async handleCookieLinkMessage(ctx: Context) {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Invalid cookie header";
      await ctx.reply(
  ...
  async sendQuizIntro(input: {
    ...
    await this.sendCurrentQuestion(input.quizId, input.telegramUserId, input.chatId);
  }

  async sendCurrentQuestion(quizId: number, telegramUserId: number, chatId: number) {
    const active = this.db.getQuizSession(quizId);
    if (!active) {
      return;
    }
  ...
  private async handleQuizAnswer(ctx: Context) {
    ...
    await this.sendCurrentQuestion(active.id, ctx.from.id, ctx.chat?.id ?? active.chatId);
  }
}
