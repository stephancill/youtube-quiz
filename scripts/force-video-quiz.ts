...
if (db.hasQuizForVideo(telegramUserId, videoId)) {
  console.log(JSON.stringify({ ok: true, message: "Quiz already exists for video" }));
  process.exit(0);
}

const activeQuizCount = db.countActiveQuizSessions(telegramUserId);
if (!ignoreActiveLimit && activeQuizCount >= MAX_ACTIVE_QUIZ_SESSIONS) {
  throw new Error(`User already has ${activeQuizCount} active quizzes. Complete one first.`);
}
...
async function generateFallbackQuestions(videoTitle: string, channelTitle: string) {
...
{"questions":[{"prompt":"...","correctAnswer":"...","sourceTimestamp":"MM:SS"}]}`;
  ...
  if (!response.ok) {
    throw new Error(`Fallback quiz generation failed: ${await response.text()}`);
  }
  ...
  if (questions.length !== 5 || questions.some((question) => !question.prompt)) {
